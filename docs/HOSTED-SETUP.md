...
## 3) حساب الدخول التجريبي

بعد تطبيق المخطط، يتوفّر حسابان جاهزان:

| الحساب | البريد | كلمة المرور | الدور |
| --- | --- | --- | --- |
| المسؤول الافتراضي | `admin@spir.local` | `admin1234` | admin |
| **التجريبي** | `demo@spir.local` | `demo1234` | admin |

الحساب التجريبي يُنشأ عبر migration `0084_demo_user.sql` تلقائيًا. **غيّر كلمات المرور
هذه فور أول دخول** (Setup/الإعدادات → المستخدمون)، أو عطّل الحساب التجريبي في الإنتاج:
...
- [ ] الدخول بـ `demo@spir.local` / `demo1234` يعمل.
