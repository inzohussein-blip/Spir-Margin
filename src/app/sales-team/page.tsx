  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-ink-gray-8">Sales Team</h1>
      ...
      <Panel title={`Sales Persons (${ps.length})`}>
        {ps.length === 0 ? <EmptyRow text="No sales persons" /> : (
      ...
      <Panel title={`Sales Partners / Channels (${pt.length})`}>
        {pt.length === 0 ? <EmptyRow text="No sales partners" /> : (
          <ul className="divide-y divide-outline-gray-1">
            {pt.map((p) => (
              <li key={p.id} className="flex items-center justify-between px-4 py-2 text-sm">
