  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-ink-gray-8">Asset Repairs</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard label="Pending" value={String(pending)} accent="amber" />
        <StatCard label="Completed cost" value={cost.toLocaleString()} accent="green" />
        <StatCard label="Total" value={String(rows.length)} accent="brand" />
      </div>

      <Panel title={`Repairs (${rows.length})`}>
        {rows.length === 0 ? (
          <EmptyRow text="No repairs yet — raise a breakdown repair for a device" />
        ) : (
