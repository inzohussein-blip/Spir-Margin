  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-ink-gray-8">Leads</h1>
        <Link href="/leads/new" className="rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-dark">
          + New lead
        </Link>
      </div>
      <Panel title={`Pipeline (${rows.length})`}>
        {rows.length === 0 ? (
          <EmptyRow text="No leads — track prospective labs before they convert" />
        ) : (
