          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-outline-gray-1">
                {rows.map((m) => (
                  <tr key={m.id}>
                    <td className="px-4 py-2 font-medium"><Link href={`/asset-movements/${m.id}`} className="text-brand hover:underline">{m.movement_no}</Link></td>
                    <td className="px-4 py-2">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${purposeBadge[m.purpose] ?? "bg-surface-gray-2"}`}>
                        {tValue(locale, m.purpose)}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-2">
                      {m.status === "draft" ? (
                        <div className="flex gap-2">
                          <ValidatedForm action={submitAssetMovementForm}>
                          ...
                          <ValidatedForm action={cancelAssetMovementForm}>
                            <input type="hidden" name="id" value={m.id} />
