    ...
    <Panel title={`${title} (${rows.length})`}>
      <div className="space-y-2 px-4 py-3">
        ...
        <form action={addMasterForm} className="flex items-center gap-2 pt-2">
          <input type="hidden" name="table" value={table} />
          <input name="name" placeholder="Add…" className="flex-1 rounded-md border border-outline-gray-2 px-2 py-1 text-sm" />
...
export default async function MastersPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-ink-gray-8">Lookup Masters</h1>
      <p className="text-sm text-ink-gray-5">Reference lists used across CRM &amp; selling forms.</p>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <NameMaster title="Sales Stages" table="sales_stages" rows={(stages as Named[]) ?? []} />
        <NameMaster title="Opportunity Types" table="opportunity_types" rows={(types as Named[]) ?? []} />
        <NameMaster title="Lost Reasons" table="opportunity_lost_reasons" rows={(reasons as Named[]) ?? []} />
      </div>

      <Panel title={`Terms & Conditions (${(terms as Term[])?.length ?? 0})`}>
        <div className="space-y-3 px-4 py-3">
          ...
          <form action={addTermForm} className="grid grid-cols-1 gap-2 pt-1 sm:grid-cols-3">
            <input name="title" placeholder="Title" className="rounded-md border border-outline-gray-2 px-2 py-1 text-sm" />
            <input name="terms" placeholder="Terms text" className="rounded-md border border-outline-gray-2 px-2 py-1 text-sm sm:col-span-2" />
