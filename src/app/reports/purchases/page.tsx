...
export default async function PurchaseSpendReport() {
  ...
  return (
    <ListShell
      ...
      breadcrumbs={[{ label: "Home", href: "/" }, { label: "Reports", href: "/reports" }, { label: "Purchase Spend" }]}
      count={rows.length}
      filterPlaceholder="Filter by supplier…"
    >
      {rows.length === 0 ? <EmptyRow text="No purchases recorded yet" /> : (
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-xs uppercase text-ink-gray-4">
              <th className="px-4 py-2">Supplier</th><th className="px-4 py-2 text-right">Invoices</th><th className="px-4 py-2 text-right">Total spend</th>
            </tr>
          </thead>
