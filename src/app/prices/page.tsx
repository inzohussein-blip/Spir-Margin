  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-ink-gray-8">Item Prices</h1>
        <Link href="/prices/new" className="rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-dark">
          + New price
        </Link>
      </div>
      <Panel title={`Price list (${rows.length})`}>
        {rows.length === 0 ? (
          <EmptyRow text="No item prices — labs fall back to the product default sell price" />
        ) : (
