    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/maintenance-schedules/new" className="inline-flex items-center gap-1.5 rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-dark"><PlusIcon size={15} /> {t(locale, "New schedule")}</Link>
        </div>
      </div>
                    ...
                    <tr key={sc.id}>
                      ...
                      <td className="px-4 py-2">
                        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusBadge[sc.status] ?? "bg-surface-gray-2"}`}>
                          {statusLabel(locale, sc.status)}
                        </span>
                      </td>
                      <td className="px-4 py-2">
                        {sc.status !== "cancelled" ? (
                          <div className="flex gap-2">
                            <ValidatedForm action={generateMaintenanceScheduleForm}>
                            ...
                            <ValidatedForm action={cancelMaintenanceScheduleForm}>
                              <input type="hidden" name="id" value={sc.id} />
