    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm text-ink-gray-5">
          {tr(locale, "Select a bank line on the left, then click a matching payment on the right.")}
        </p>
        <button
          onClick={autoMatch}
          disabled={pending}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-outline-gray-2 bg-surface-white">
          <div className="border-b border-outline-gray-1 px-4 py-2 text-sm font-semibold text-ink-gray-7">
            {tr(locale, "Unreconciled transactions")} ({transactions.length})
          </div>
          <ul className="divide-y divide-outline-gray-1">
            {transactions.length === 0 && (
              <li className="px-4 py-6 text-center text-sm text-ink-gray-4">{tr(locale, "All reconciled")} 🎉</li>
            )}
            {transactions.map((t) => (
              <li key={t.id}>
                <button
        ...
        <div className="rounded-xl border border-outline-gray-2 bg-surface-white">
          <div className="border-b border-outline-gray-1 px-4 py-2 text-sm font-semibold text-ink-gray-7">
            {tr(locale, "Open payments")} ({payments.length})
          </div>
          <ul className="divide-y divide-outline-gray-1">
            {candidates.length === 0 && (
              <li className="px-4 py-6 text-center text-sm text-ink-gray-4">{tr(locale, "No open payments")}</li>
            )}
            {candidates.map((p) => {
