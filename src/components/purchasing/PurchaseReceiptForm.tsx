...
export function PurchaseReceiptForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <Card>
        <CardHeader><CardTitle>{t(locale, "Purchase Receipt")}</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2 text-sm">
          <label className="block">
            <span className="font-medium text-ink-gray-8">{t(locale, "Receipt no.")}</span>
      ...
      <Card>
        <CardHeader><CardTitle>{t(locale, "Items received")}</CardTitle></CardHeader>
