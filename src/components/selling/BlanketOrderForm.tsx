...
export function BlanketOrderForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <Card>
        <CardHeader><CardTitle>{t(locale, "Blanket Order")}</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2 text-sm">
          <label className="block">
            <span className="font-medium text-ink-gray-8">{t(locale, "Order no.")}</span>
            <input {...register("order_no")} className={cls} placeholder={t(locale, "auto if blank")} />
          </label>
          <label className="block">
      ...
      <Card>
        <CardHeader><CardTitle>{t(locale, "Agreed items")}</CardTitle></CardHeader>
            ...
            <div key={f.id} className="rounded-lg border border-outline-gray-1 p-3">
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-6">
                ...
                <label className="block text-xs">
                  <span className="text-ink-gray-5">{t(locale, "Agreed qty")}</span>
      ...
      <Button type="submit" variant="solid" size="md" disabled={pending}>
        ...
        {t(locale, "Create blanket order (draft)")}
      </Button>
    </form>
  );
}
