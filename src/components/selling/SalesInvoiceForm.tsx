...
export function SalesInvoiceForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <Card>
        <CardHeader><CardTitle>{t(locale, "Sales Invoice")}</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2 text-sm">
          <label className="block">
            <span className="font-medium text-ink-gray-8">{t(locale, "Invoice no.")}</span>
            <input {...register("invoice_no")} className={cls} placeholder="SI-0001" />
          </label>
          <label className="block">
          ...
          <label className="block">
            <span className="font-medium text-ink-gray-8">{t(locale, "Due date")}</span>
            <input type="date" lang="en-CA" {...register("due_date")} className={cls} />
          </label>
