-- =====================================================================
-- Spir-Margin — combined schema (all 57 migrations + seed)
--
-- One-shot bootstrap for a FRESH Supabase / Postgres database.
-- Paste this whole file into Supabase -> SQL Editor -> Run, ONE TIME.
--
-- Run this ONCE, on an EMPTY project. A few views are redefined across
-- migrations, so replaying the whole file is not supported — the single first
-- run is what matters, and afterwards the app applies any new migrations itself.
-- Do NOT run this against a database that already holds another app (it will
-- collide with existing tables such as users / appointments / payments).
-- Generated from supabase/migrations/*.sql + supabase/seed.sql — do not edit by hand.
-- =====================================================================

-- gen_random_uuid() lives in core on PG13+, but keep pgcrypto available too.
create extension if not exists pgcrypto;

-- RLS policies reference the "authenticated" role (already present on Supabase).
do $$ begin
