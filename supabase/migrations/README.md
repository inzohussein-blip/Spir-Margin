...
3. If it adds a trigger, end the file with `select _spir_guard_triggers();` so
   the trigger does not fire again on changes that arrive by sync (0106).
   If it adds a table, end the file with `select _spir_attach_change_log();`
